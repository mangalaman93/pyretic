...
#Command to execute: sudo mn --custom conflictTopo.py --topo mytopo --switch ovsk --link tc --controller=remote
...
class FVTopo(Topo):
	...
	self.addLink('s1', 's2', port1=2, port2=1, **http_link_config)
        ...
        self.addLink('s1', 's7', port1=4, port2=1, **video_link_config)
	...
	self.addLink('s3', 's5', port1=2, port2=2, **video_link_config)
        self.addLink('s3', 's4', port1=3, port2=1, **http_link_config)
       
        self.addLink('s4', 's6', port1=2, port2=2, **video_link_config)
        self.addLink('s4', 's9', port1=3, port2=2, **video_link_config) 
	self.addLink('s4', 'h2', port1=4, port2=1, **video_link_config)

        self.addLink('s7', 's8', port1=2, port2=1, **video_link_config)      
